...
rocket = rocketclass(
    np.array([400.0, 400.0]), np.array([0.0, 0.0]), (255, 255, 255), 6479, 1000, 10000
)
top = lander(
    np.array(
        [
...
def updateVect(top, bottom, split: bool, thrustvalue: float, crash: bool, pressed: dict):
    ...
    for i in range(8):
        for key in [
            ...
            (pressed["left"], 0, 1 / 2),
            (pressed["right"], 0, -1 / 2),
            (True if top.vertices[7][1] < 685 else False, 1, -G / finalthrust),
        ]:
            if key[0]:
                for obj in objlist:
...
def drawall(top, bottom, split: bool, pressed: dict):
    ...
    if pressed["thrust"]:
        pygame.draw.polygon(
            ...
            (
                [
                    (top.vertices[1][0], top.vertices[1][1] + 55),
                    (top.vertices[1][0] - 30, top.vertices[1][1] + 55),
                    (top.vertices[1][0] - 15, top.vertices[1][1] + 100),
                ]
            ),
        )
...
def drawaftercrash(num: int, downforce: float, sideforce: float):
    ...
    if num < 200:
        ...
        for num2, color in [
            ...
            if num > num2:
                pygame.draw.circle(
                    screen, color, (top.vertices[1][0] - 15, 670), num - num2
                )
    ...
    if phase == "p1":
        ...
        if crash == False:
            if split == False:
                ...
                split, crash, downforce, sideforce = updateVect(
                    top, bottom, split, thrustvalue, crash, pressed
                )
