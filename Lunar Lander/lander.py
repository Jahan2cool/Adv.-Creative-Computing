...
class landerclass:
    ...
    def updateVect(this, split: bool, thrustvalue: float, crash: bool, pressed: dict):
    ...
    def draw(this, split: bool, pressed: dict):
        ...
        if pressed["thrust"]:
            pygame.draw.polygon(screen,(255, 0, 0),([(this.top.vertices[1][0], this.top.vertices[1][1] + 55),(this.top.vertices[1][0] - 30, this.top.vertices[1][1] + 55),(this.top.vertices[1][0] - 15, this.top.vertices[1][1] + 100)]))
        for x in [(pressed["left"], 2, 3, 1), (pressed["right"], 7, 6, -1)]:
            if x[0]:
                pygame.draw.polygon(screen,(255, 0, 0),([(this.top.vertices[x[1]][0], this.top.vertices[x[1]][1] + 10),(this.top.vertices[x[2]][0], this.top.vertices[x[2]][1] - 10),(this.top.vertices[x[1]][0] + 10 * x[3], this.top.vertices[x[1]][1] + 15),]))
    ...
    def drawaftercrash(this, num: int, downforce: float, sideforce: float):
        screen.fill(black)
        ...
        if num < 200:
            ...
            for num2, color in [(0, red),(30, orange),(100, yellow)]:
                if num > num2:
                    pygame.draw.circle(screen, color, (this.top.vertices[1][0] - 15, 670), num - num2)
...
while run:
    ...
    if phase == "p1":
        ...
        if crash == False:
            if split == False:
                pressed = lander.bottom.fuelcheck(keys)

                split, crash, downforce, sideforce = lander.updateVect(split, thrustvalue, crash, pressed)
            ...
            # A check to make sure you reset you w key so the ascent dosen't immedietly start
                ...
                pressed = lander.top.fuelcheck(keys)
